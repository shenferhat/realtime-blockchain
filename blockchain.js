...
class Transaction {
    constructor(fromAddress, toAddress, amount, payload) {
        ...
        this.payload = Buffer.from(String(payload), 'binary').toString('base64');
    }
    ...
    calculateHash() {
        return SHA256(this.fromAddress + this.toAddress, +this.amount + this.timestamp + this.payload).toString();
    }

    toStr() {
        return "From Address:" + this.fromAddress + " To Address: " + this.toAddress + " Amount:" + this.amount + " Payload:" + this.payload + " Timestamp:" + this.timestamp;
    }
...
class Blockchain {
    ...
    printTransactions() {
        ...
        for (let i = 1; i < this.chain.length; i++) {
            const currentBlock = this.chain[i];
            a.push(currentBlock.transactions);
        }
        return a;
    }
    ...
    calculateTotalAmount() {
        ...
        for (let i = 1; i < this.chain.length; i++) {
            const currentBlock = this.chain[i];
            sum += currentBlock.transactions.length;
        }
        return (sum / 2) * 10;
    }
}
